# Aggregate Table Materialization — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Materializes governed nightly and intraday aggregate tables so common analytics remain fast and reproducible.
Topics: openchart-feature-catalog, analytics, frappe, materialization
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-040 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Dependency-Aware Backfills Can Rebuild Only** — dependency-aware backfills can rebuild only affected partitions after source corrections.

## Focus

This feature isolates one capability: materializes governed nightly and intraday aggregate tables so common analytics remain fast and reproducible. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Analytics Engineers start from dataset definitions, grains, dependencies, watermarks, schedules, and retention.
...
- A successful run produces versioned aggregate partitions with row counts, checksums, freshness, and lineage.
...
- Partial or inconsistent builds remain quarantined while the last healthy partition serves.
...
- **DocTypes:** `OC Aggregate Build` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
