# Revenue Performance Dashboard — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Tracks charges, payments, accounts-receivable aging, and denial rates by payer and service line.
Topics: openchart-feature-catalog, analytics, frappe, revenue-dashboard
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-019 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Forecast Bands Can Model Collection** — forecast bands can model collection timing without altering source receivables.

## Focus

This feature isolates one capability: tracks charges, payments, accounts-receivable aging, and denial rates by payer and service line. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Revenue Cycle Leaders start from governed financial extracts, payer mappings, posting dates, and reconciliation state.
...
- A successful run produces trendable revenue measures with aging buckets and payer drill-down.
...
- Unbalanced extracts or unmapped payers are isolated from certified totals.
...
- **DocTypes:** `OC Revenue Metric Snapshot` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
