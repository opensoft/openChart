# Semantic Metric Registry — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Publishes one governed, versioned definition for every KPI so internal and external tools calculate the same measure.
Topics: openchart-feature-catalog, analytics, frappe, metric-registry
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-001 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Metric Compatibility Tests Compare Reference** — metric compatibility tests compare reference inputs across Frappe and external BI engines.

## Focus

This feature isolates one capability: publishes one governed, versioned definition for every KPI so internal and external tools calculate the same measure. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Analytics Stewards start from a metric name, formula, grain, dimensions, exclusions, owner, and effective dates.
...
- A successful run produces an immutable public definition version with machine-readable semantics.
...
- Conflicting identifiers or overlapping effective dates block publication.
...
- **DocTypes:** `OC Metric Definition` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
