# Board Pack Assembly — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Assembles selected dashboard snapshots, commentary, and appendices into a reproducible print or PDF board pack.
Topics: openchart-feature-catalog, analytics, frappe, board-packs
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-039 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Approval Signatures Can Attest The** — approval signatures can attest the exact pack version distributed.

## Focus

This feature isolates one capability: assembles selected dashboard snapshots, commentary, and appendices into a reproducible print or PDF board pack. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Executive Analysts start from certified widgets, reporting period, narrative annotations, ordering, and letterhead.
...
- A successful run produces a frozen PDF pack with contents, page numbers, definitions, and source run IDs.
...
- Stale widgets or changed permissions stop regeneration and identify affected pages.
...
- **DocTypes:** `OC Board Pack` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
