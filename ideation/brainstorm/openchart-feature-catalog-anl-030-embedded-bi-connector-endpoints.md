# Embedded BI Connector Endpoints — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Offers documented flat-extract endpoints so open-source BI tools can connect without proprietary semantic lock-in.
Topics: openchart-feature-catalog, analytics, frappe, bi-connectors
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-030 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Connector Profiles Can Package Examples** — connector profiles can package examples for common open-source BI platforms.

## Focus

This feature isolates one capability: offers documented flat-extract endpoints so open-source BI tools can connect without proprietary semantic lock-in. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- BI Administrators start from approved dataset, dimensions, incremental cursor, output schema, and client identity.
...
- A successful run produces a stable, permission-filtered flat extract with schema and watermark.
...
- Schema changes require a new endpoint version and invalid cursors fail explicitly.
...
- **DocTypes:** `OC BI Connector` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
