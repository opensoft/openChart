# Risk Stratification Score Display — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Displays imported or computed risk scores with model version, inputs, confidence, and intended-use limits.
Topics: openchart-feature-catalog, analytics, frappe, risk-stratification
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-024 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Model-Monitoring Panels Can Compare Calibration** — model-monitoring panels can compare calibration across approved subgroups.

## Focus

This feature isolates one capability: displays imported or computed risk scores with model version, inputs, confidence, and intended-use limits. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Care Managers and Clinicians start from a governed model version, score, contributing factors, timestamps, and applicability.
...
- A successful run produces an advisory score display with explanation and historical trend.
...
- Expired, out-of-domain, or incomplete model results are visibly invalidated.
...
- **DocTypes:** `OC Risk Score` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
