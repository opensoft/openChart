# Metric Lineage Explorer — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Traces a rendered value from metric definition through datasets, transformations, query execution, and dashboard widget.
Topics: openchart-feature-catalog, analytics, frappe, metric-lineage
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-004 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Lineage Diffs Explain Why The** — lineage diffs explain why the same metric changed between two report runs.

## Focus

This feature isolates one capability: traces a rendered value from metric definition through datasets, transformations, query execution, and dashboard widget. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Auditors and Report Authors start from a metric result, report run, or dashboard widget identifier.
...
- A successful run produces a navigable provenance chain with definition versions and execution timestamps.
...
- Broken or unavailable lineage nodes are shown explicitly rather than inferred.
...
- **DocTypes:** `OC Metric Lineage` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
