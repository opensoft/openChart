# Cohort Sharing And Versioning — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Makes cohort definitions shareable without allowing collaborators to silently change published analytical meaning.
Topics: openchart-feature-catalog, analytics, frappe, cohort-sharing
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-006 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **A Compare View Highlights Semantic** — a compare view highlights semantic changes between cohort versions.

## Focus

This feature isolates one capability: makes cohort definitions shareable without allowing collaborators to silently change published analytical meaning. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Cohort Owners and Collaborators start from a saved cohort, audience, permission level, and revision rationale.
...
- A successful run produces a versioned shared definition with owner, viewers, editors, and succession history.
...
- Revoked access removes future visibility while preserving prior run evidence.
...
- **DocTypes:** `OC Cohort Share` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
