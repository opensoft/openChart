# Ad-hoc Query Builder — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Lets nontechnical users assemble governed tabular reports with filters, groups, measures, and visual previews.
Topics: openchart-feature-catalog, analytics, frappe, query-builder
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-008 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Suggested Joins Explain Cardinality And** — suggested joins explain cardinality and duplication risks before execution.

## Focus

This feature isolates one capability: lets nontechnical users assemble governed tabular reports with filters, groups, measures, and visual previews. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Operational Analysts start from catalog fields, joins exposed by datasets, filters, groupings, and measures.
...
- A successful run produces a saved report definition and permission-filtered result preview.
...
- Ambiguous joins, incompatible grains, or cost limits return actionable errors.
...
- **DocTypes:** `OC Ad Hoc Report` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
