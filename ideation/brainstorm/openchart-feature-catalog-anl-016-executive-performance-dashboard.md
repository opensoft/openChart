# Executive Performance Dashboard — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Summarizes access, quality, finance, workforce, and equity outcomes for leadership using certified metric versions.
Topics: openchart-feature-catalog, analytics, frappe, executive-dashboard
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-016 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Narrative Annotations Can Preserve Management** — narrative annotations can preserve management context alongside each reporting period.

## Focus

This feature isolates one capability: summarizes access, quality, finance, workforce, and equity outcomes for leadership using certified metric versions. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Executives and Board Analysts start from certified metrics, targets, comparison periods, and organization scope.
...
- A successful run produces a concise scorecard with trends, variance explanations, and linked evidence.
...
- Suppressed cells and provisional data remain visibly qualified in every rollup.
...
- **DocTypes:** `OC Executive Dashboard` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
