# Permission-scoped SQL Console — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Provides power users a read-only SQL workspace over approved analytics views with enforced scope and resource limits.
Topics: openchart-feature-catalog, analytics, frappe, sql-console
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-009 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Named Query Snippets Can Be** — named query snippets can be reviewed and promoted into governed reports.

## Focus

This feature isolates one capability: provides power users a read-only SQL workspace over approved analytics views with enforced scope and resource limits. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Analytics Engineers start from a SQL statement, approved dataset connections, and execution parameters.
...
- A successful run produces a bounded result set, query plan, cost estimate, and immutable execution record.
...
- Writes, unsafe functions, unapproved tables, or limit breaches are rejected before execution.
...
- **DocTypes:** `OC SQL Workspace` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
