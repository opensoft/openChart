# Payer Mix Trend Analysis — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Tracks payer composition across patients, encounters, charges, and receipts using versioned payer groupings.
Topics: openchart-feature-catalog, analytics, frappe, payer-mix
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-051 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Contract-Level Views Can Remain Restricted** — contract-level views can remain restricted to authorized finance audiences.

## Focus

This feature isolates one capability: tracks payer composition across patients, encounters, charges, and receipts using versioned payer groupings. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Finance and Strategy Analysts start from payer identities, mapping versions, selected grain, periods, and organization scope.
...
- A successful run produces mix percentages, counts, amounts, and change indicators with unmapped categories.
...
- Mapping changes create explicit breaks or restated series rather than silent rewrites.
...
- **DocTypes:** `OC Payer Mix Snapshot` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
