# Custom Dashboard Builder — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Lets authorized users compose dashboards from governed widgets without bypassing metric semantics or access rules.
Topics: openchart-feature-catalog, analytics, frappe, dashboard-builder
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-037 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **A Widget Library Can Include** — a widget library can include tables, trends, funnels, cohorts, maps, and narrative panels.

## Focus

This feature isolates one capability: lets authorized users compose dashboards from governed widgets without bypassing metric semantics or access rules. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Dashboard Authors start from certified reports, metrics, filters, layout, refresh cadence, and audience intent.
...
- A successful run produces a draft or published dashboard with responsive layout and dependency manifest.
...
- Incompatible filters, uncertified content, or forbidden audience scopes block publication.
...
- **DocTypes:** `OC Analytics Dashboard` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
