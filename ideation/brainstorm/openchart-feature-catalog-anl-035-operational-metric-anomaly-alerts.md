# Operational Metric Anomaly Alerts — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Detects unusual volume, rate, or latency changes and routes explainable alerts for human investigation.
Topics: openchart-feature-catalog, analytics, frappe, anomaly-detection
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-035 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Feedback Labels Can Tune Sensitivity** — feedback labels can tune sensitivity without rewriting historical alerts.

## Focus

This feature isolates one capability: detects unusual volume, rate, or latency changes and routes explainable alerts for human investigation. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Operations Managers start from certified metric series, seasonality rules, thresholds, and notification routing.
...
- A successful run produces an alert with baseline, observed value, confidence, affected dimensions, and status.
...
- Insufficient history suppresses detection and feed gaps create data-quality alerts instead.
...
- **DocTypes:** `OC Metric Anomaly` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
