# Campaign Effectiveness Attribution — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Measures outreach-to-response-to-appointment conversion with explicit attribution windows and comparison logic.
Topics: openchart-feature-catalog, analytics, frappe, campaign-attribution
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-045 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Holdout Cohorts Can Support Reviewed** — holdout cohorts can support reviewed effectiveness studies where appropriate.

## Focus

This feature isolates one capability: measures outreach-to-response-to-appointment conversion with explicit attribution windows and comparison logic. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Engagement Analysts start from campaign membership, delivery events, responses, appointments, outcomes, and attribution rules.
...
- A successful run produces a funnel with conversion, lag, exclusions, and attributable versus unattributed outcomes.
...
- Duplicate identities, overlapping campaigns, and missing consent are surfaced as exclusions.
...
- **DocTypes:** `OC Campaign Analysis` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
