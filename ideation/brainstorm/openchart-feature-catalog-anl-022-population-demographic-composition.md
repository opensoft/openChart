# Population Demographic Composition — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Renders population pyramids and demographic composition while protecting small or sensitive groups.
Topics: openchart-feature-catalog, analytics, frappe, demographics
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-022 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Custom Band Sets Can Support** — custom band sets can support local planning while retaining canonical defaults.

## Focus

This feature isolates one capability: renders population pyramids and demographic composition while protecting small or sensitive groups. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Population Health Teams start from authorized population scope, age bands, sex, race, language, insurance, and as-of date.
...
- A successful run produces suppression-aware distributions and trends with denominator definitions.
...
- Unknown values remain explicit and small cells are masked consistently.
...
- **DocTypes:** `OC Population Snapshot` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
