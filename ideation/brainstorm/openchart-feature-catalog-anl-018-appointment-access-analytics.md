# Appointment Access Analytics — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Measures fill rate, no-shows, cancellations, utilization, and time-to-third-available across scheduling dimensions.
Topics: openchart-feature-catalog, analytics, frappe, appointment-analytics
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-018 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Cancellation-Backfill Analysis Can Distinguish Recovered** — cancellation-backfill analysis can distinguish recovered from lost capacity.

## Focus

This feature isolates one capability: measures fill rate, no-shows, cancellations, utilization, and time-to-third-available across scheduling dimensions. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Access Managers start from slot supply, booking transitions, appointment outcomes, and calendar snapshots.
...
- A successful run produces certified access metrics with provider, facility, type, and period breakdowns.
...
- Missing capacity snapshots prevent retrospective availability claims and are flagged.
...
- **DocTypes:** `OC Access Metric Snapshot` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
