# External Benchmark Ingestion — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Imports external baselines with licensing, cohort, period, and methodology metadata for valid comparisons.
Topics: openchart-feature-catalog, analytics, frappe, benchmarks
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-036 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Crosswalk Reports Can Expose Where** — crosswalk reports can expose where local and external definitions diverge.

## Focus

This feature isolates one capability: imports external baselines with licensing, cohort, period, and methodology metadata for valid comparisons. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Analytics Stewards start from benchmark files or APIs, source terms, measure mappings, populations, and periods.
...
- A successful run produces a versioned benchmark release mapped to compatible internal metrics.
...
- Incompatible definitions or missing rights prevent comparison publication.
...
- **DocTypes:** `OC Benchmark Release` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
