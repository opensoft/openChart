# Provider Productivity Measures — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Calculates encounters, worked time, and configurable wrRVU-style productivity under published attribution rules.
Topics: openchart-feature-catalog, analytics, frappe, productivity
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-020 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Compensation Exports Can Require Separate** — compensation exports can require separate approval and explicit policy versions.

## Focus

This feature isolates one capability: calculates encounters, worked time, and configurable wrRVU-style productivity under published attribution rules. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Providers and Workforce Planners start from encounters, procedures, schedules, attribution, and weight tables.
...
- A successful run produces periodic productivity measures with denominators, exceptions, and source lineage.
...
- Unversioned weights or ambiguous attribution mark results provisional.
...
- **DocTypes:** `OC Productivity Snapshot` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
