# Episode Cost-of-care Views — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Groups governed clinical and claims costs into transparent episodes for service-line and value analysis.
Topics: openchart-feature-catalog, analytics, frappe, cost-of-care
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-050 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Scenario Views Can Test Alternative** — scenario views can test alternative episode windows without changing certified results.

## Focus

This feature isolates one capability: groups governed clinical and claims costs into transparent episodes for service-line and value analysis. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Value-based Care and Finance Analysts start from episode definitions, claims, charges, allowed amounts, clinical events, and attribution.
...
- A successful run produces episode totals, components, variation, and comparators with definition lineage.
...
- Overlapping episodes and missing cost bases are classified instead of silently allocated.
...
- **DocTypes:** `OC Cost Episode` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
