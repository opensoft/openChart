# Saved Report Catalog — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Organizes governed and personal reports into a searchable catalog with ownership, certification, and freshness cues.
Topics: openchart-feature-catalog, analytics, frappe, report-catalog
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-010 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Usage Signals Can Identify Duplicate** — usage signals can identify duplicate reports for steward-led consolidation.

## Focus

This feature isolates one capability: organizes governed and personal reports into a searchable catalog with ownership, certification, and freshness cues. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- All Reporting Users start from report definitions, owners, tags, certification state, and audience.
...
- A successful run produces a permission-filtered catalog with favorites, recent runs, and endorsed results.
...
- Retired reports remain discoverable through lineage but cannot be newly scheduled.
...
- **DocTypes:** `OC Report Definition` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
