# Analytics Data Quality Dashboard — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Surfaces missing, stale, invalid, duplicated, and unmapped data before defects contaminate decisions.
Topics: openchart-feature-catalog, analytics, frappe, data-quality
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-027 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Accepted Exceptions Can Carry Owners** — accepted exceptions can carry owners and expiry dates rather than hiding defects.

## Focus

This feature isolates one capability: surfaces missing, stale, invalid, duplicated, and unmapped data before defects contaminate decisions. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Data Stewards and Source Owners start from quality rules, dataset runs, freshness thresholds, and ownership mappings.
...
- A successful run produces severity-ranked quality measures with trends and accountable issue drill-down.
...
- Rule execution failures appear separately from passing or failing data.
...
- **DocTypes:** `OC Data Quality Rule` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
