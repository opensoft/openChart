# Spreadsheet And CSV Export — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Exports permission-filtered report results to CSV or XLSX with definitions and provenance attached.
Topics: openchart-feature-catalog, analytics, frappe, data-export
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-028 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Watermarked Workbooks Can Include A** — watermarked workbooks can include a read-me sheet describing metric semantics.

## Focus

This feature isolates one capability: exports permission-filtered report results to CSV or XLSX with definitions and provenance attached. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Authorized Report Consumers start from a completed report run, selected columns, format, and locale.
...
- A successful run produces a bounded file with labels, types, filters, definition versions, and expiry.
...
- Large, suppressed, or newly restricted results are rejected or routed to background export.
...
- **DocTypes:** `OC Report Export` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
