# Cohort Patient Drill-down — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Lets clinically authorized users move from an aggregate cohort count to the exact patient worklist supporting it.
Topics: openchart-feature-catalog, analytics, frappe, cohort-drilldown
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-007 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Care-Manager Assignments Can Be Created** — care-manager assignments can be created from a reviewed drill-down selection.

## Focus

This feature isolates one capability: lets clinically authorized users move from an aggregate cohort count to the exact patient worklist supporting it. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Care Managers and Authorized Clinicians start from a cohort run, selected segment, and permitted patient scope.
...
- A successful run produces a paginated patient list with inclusion reasons and current source links.
...
- Small-cell suppression, consent, and user permissions can deny or narrow drill-down.
...
- **DocTypes:** `OC Cohort Run` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
