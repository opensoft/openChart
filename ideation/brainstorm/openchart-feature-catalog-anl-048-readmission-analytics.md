# Readmission Analytics — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Measures configurable readmission episodes with index-event, exclusion, attribution, and follow-up transparency.
Topics: openchart-feature-catalog, analytics, frappe, readmissions
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-048 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Pre-Close Case Review Can Resolve** — pre-close case review can resolve data defects before a reporting period is certified.

## Focus

This feature isolates one capability: measures configurable readmission episodes with index-event, exclusion, attribution, and follow-up transparency. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Quality Analysts start from admissions, discharges, encounters, exclusions, attribution, and interval rules.
...
- A successful run produces readmission rates, case lists, trends, and reason-group breakdowns.
...
- Incomplete external utilization data qualifies denominators and comparisons.
...
- **DocTypes:** `OC Readmission Episode` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
