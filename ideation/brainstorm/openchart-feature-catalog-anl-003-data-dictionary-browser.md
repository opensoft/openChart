# Analytics Data Dictionary Browser — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Lets authorized users inspect catalog fields, meanings, types, provenance, freshness, and permitted uses without reading database schema.
Topics: openchart-feature-catalog, analytics, frappe, data-dictionary
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-003 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Steward Annotations Can Explain Local** — steward annotations can explain local coding conventions without changing source metadata.

## Focus

This feature isolates one capability: lets authorized users inspect catalog fields, meanings, types, provenance, freshness, and permitted uses without reading database schema. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Report Authors and Data Stewards start from a field search, dataset filter, or business concept.
...
- A successful run produces a permission-filtered field card with definition, source, examples, and sensitivity.
...
- Restricted fields appear only as redacted catalog entries and stale metadata is flagged.
...
- **DocTypes:** `OC Analytics Field` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
