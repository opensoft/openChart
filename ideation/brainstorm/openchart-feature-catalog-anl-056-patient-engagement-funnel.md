# Patient Engagement Funnel — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Measures portal invitation, activation, use, message response, and appointment conversion as an explicit funnel.
Topics: openchart-feature-catalog, analytics, frappe, engagement-analytics
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-056 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Journey Segments Can Distinguish Onboarding** — journey segments can distinguish onboarding from ongoing portal use.

## Focus

This feature isolates one capability: measures portal invitation, activation, use, message response, and appointment conversion as an explicit funnel. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Patient Engagement Teams start from portal, messaging, campaign, appointment, and response events with consent scope.
...
- A successful run produces stage counts, conversion, abandonment, lag, and demographic stratification.
...
- Identity linkage gaps and channel opt-outs remain explicit exclusions.
...
- **DocTypes:** `OC Engagement Funnel Snapshot` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
