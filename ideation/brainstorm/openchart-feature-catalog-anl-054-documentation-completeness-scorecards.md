# Documentation Completeness Scorecards — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Measures required documentation elements by provider and encounter type while preserving context and correction status.
Topics: openchart-feature-catalog, analytics, frappe, documentation-analytics
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-054 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Pre-Sign Feedback Can Remain Distinct** — pre-sign feedback can remain distinct from retrospective performance reporting.

## Focus

This feature isolates one capability: measures required documentation elements by provider and encounter type while preserving context and correction status. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Clinical Documentation Leaders and Providers start from encounter requirements, signed documents, required elements, exceptions, and periods.
...
- A successful run produces provider-private completeness trends with denominators and missing-element drill-down.
...
- Amended or exempt records are classified under the effective policy version.
...
- **DocTypes:** `OC Documentation Score` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
