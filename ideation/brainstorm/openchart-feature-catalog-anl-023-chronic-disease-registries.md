# Chronic Disease Registries — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Maintains condition-specific cohorts and control-rate trends for longitudinal population management.
Topics: openchart-feature-catalog, analytics, frappe, disease-registries
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-023 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Registry Templates Can Package Reviewed** — registry templates can package reviewed definitions for common chronic conditions.

## Focus

This feature isolates one capability: maintains condition-specific cohorts and control-rate trends for longitudinal population management. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Care Managers and Quality Leads start from versioned condition criteria, observations, medications, care gaps, and time windows.
...
- A successful run produces a registry dashboard with control status, trends, and authorized patient worklists.
...
- Insufficient or stale clinical evidence yields unknown rather than uncontrolled status.
...
- **DocTypes:** `OC Disease Registry` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
