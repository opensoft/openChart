# SDOH Geographic Heat Maps — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Maps aggregate social-needs and community context measures by geography without exposing patient locations.
Topics: openchart-feature-catalog, analytics, frappe, sdoh-heatmaps
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-025 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Community-Resource Overlays Can Support Planning** — community-resource overlays can support planning without implying causal effects.

## Focus

This feature isolates one capability: maps aggregate social-needs and community context measures by geography without exposing patient locations. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Population Health Planners start from geocoded aggregate areas, SDOH indicators, time periods, and suppression policy.
...
- A successful run produces a choropleth with legends, uncertainty, freshness, and permitted drill levels.
...
- Small cells, low geocoding quality, or prohibited boundaries prevent detailed rendering.
...
- **DocTypes:** `OC Geographic Indicator` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
