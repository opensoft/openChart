# Panel Size And Capacity Planning — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Relates attributed panel demand to provider capacity so leaders can model sustainable access scenarios.
Topics: openchart-feature-catalog, analytics, frappe, capacity-planning
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-021 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Scenario Comparison Can Expose The** — scenario comparison can expose the access effect of staffing or template changes.

## Focus

This feature isolates one capability: relates attributed panel demand to provider capacity so leaders can model sustainable access scenarios. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Workforce Planners start from panel attribution, risk mix, visit demand, schedules, and scenario assumptions.
...
- A successful run produces current-state measures and nonbinding capacity scenarios with assumption labels.
...
- Stale attribution or impossible assumptions block scenario publication.
...
- **DocTypes:** `OC Capacity Scenario` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
