# Clinical Operations Dashboard — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Combines encounter flow, unsigned work, results follow-up, and care-gap workload for clinical operations oversight.
Topics: openchart-feature-catalog, analytics, frappe, clinical-dashboard
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-014 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Team-Level Workload Balancing Suggestions Can** — team-level workload balancing suggestions can remain advisory and human approved.

## Focus

This feature isolates one capability: combines encounter flow, unsigned work, results follow-up, and care-gap workload for clinical operations oversight. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Clinical Operations Leads start from authorized encounter, documentation, result, and task aggregates.
...
- A successful run produces a role-scoped dashboard with workload trends and accountable drill-through.
...
- Patient-level links are hidden when the viewer lacks clinical relationship or facility scope.
...
- **DocTypes:** `OC Clinical Operations Dashboard` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
