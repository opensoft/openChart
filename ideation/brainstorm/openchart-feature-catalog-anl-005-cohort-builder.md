# Saved Cohort Builder — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Enables no-SQL construction of reusable patient cohorts from governed clinical, demographic, and operational criteria.
Topics: openchart-feature-catalog, analytics, frappe, cohort-builder
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-005 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Reusable Criterion Templates Accelerate Common** — reusable criterion templates accelerate common registry and quality-measure cohorts.

## Focus

This feature isolates one capability: enables no-SQL construction of reusable patient cohorts from governed clinical, demographic, and operational criteria. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Population Health Analysts start from nested inclusion, exclusion, date-window, and value criteria.
...
- A successful run produces a validated cohort definition with count preview and explicit as-of semantics.
...
- Invalid criteria, forbidden fields, or excessive estimated cost prevent execution.
...
- **DocTypes:** `OC Cohort Definition` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
