# Target Versus Actual Tracking — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Tracks approved goals against certified actuals with ownership, cadence, and variance explanations.
Topics: openchart-feature-catalog, analytics, frappe, goal-tracking
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-044 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Forecast Scenarios Can Remain Distinct** — forecast scenarios can remain distinct from committed targets.

## Focus

This feature isolates one capability: tracks approved goals against certified actuals with ownership, cadence, and variance explanations. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Goal Owners and Leaders start from a metric version, target value or trajectory, scope, period, owner, and rationale.
...
- A successful run produces a status series with variance, confidence, commentary, and review state.
...
- Targets cannot be silently restated after a period closes and incompatible actuals are rejected.
...
- **DocTypes:** `OC Metric Target` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
