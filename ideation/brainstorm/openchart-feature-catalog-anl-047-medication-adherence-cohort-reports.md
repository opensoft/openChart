# Medication Adherence Cohort Reports — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Reports refill- or observation-based adherence proxies with transparent evidence limits and patient worklists.
Topics: openchart-feature-catalog, analytics, frappe, medication-adherence
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-047 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Outreach Outcomes Can Be Measured** — outreach outcomes can be measured without making autonomous medication decisions.

## Focus

This feature isolates one capability: reports refill- or observation-based adherence proxies with transparent evidence limits and patient worklists. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Pharmacy Teams and Care Managers start from medication statements, dispense or claims events, coverage windows, and cohort criteria.
...
- A successful run produces adherence distributions, trends, unknown states, and authorized drill-down.
...
- Insufficient evidence never labels a patient nonadherent and conflicting sources are flagged.
...
- **DocTypes:** `OC Adherence Analysis` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
