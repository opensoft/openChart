# Report Subscription Management — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Lets users subscribe, pause, resume, and unsubscribe from reports without changing the owner's report definition.
Topics: openchart-feature-catalog, analytics, frappe, report-subscriptions
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-012 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Digest Mode Can Combine Several** — digest mode can combine several low-priority reports into one delivery.

## Focus

This feature isolates one capability: lets users subscribe, pause, resume, and unsubscribe from reports without changing the owner's report definition. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Report Consumers start from a subscribable report, preferred cadence, format, and authorized parameter values.
...
- A successful run produces a personal active, paused, expired, or revoked subscription.
...
- Access loss automatically revokes future deliveries and records the reason.
...
- **DocTypes:** `OC Report Subscription` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
