# Telehealth Utilization Analysis — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Compares telehealth and in-person use, completion, access, outcomes, and modality shifts.
Topics: openchart-feature-catalog, analytics, frappe, telehealth-analytics
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-057 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Connection-Failure Outcomes Can Explain Avoidable** — connection-failure outcomes can explain avoidable modality conversion.

## Focus

This feature isolates one capability: compares telehealth and in-person use, completion, access, outcomes, and modality shifts. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Digital Care Leaders start from appointment modality, encounter completion, patient geography, provider, and period.
...
- A successful run produces utilization splits and trends with access and outcome comparators.
...
- Modality changes and hybrid encounters follow published classification rules.
...
- **DocTypes:** `OC Telehealth Snapshot` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
