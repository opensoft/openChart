# Front-desk Operations Dashboard — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Shows registration, arrival, queue, scheduling, and unresolved intake measures needed to coordinate the current clinic day.
Topics: openchart-feature-catalog, analytics, frappe, front-desk-dashboard
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-013 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Shift Handoff Snapshots Preserve Unresolved** — shift handoff snapshots preserve unresolved operational context.

## Focus

This feature isolates one capability: shows registration, arrival, queue, scheduling, and unresolved intake measures needed to coordinate the current clinic day. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Front Desk Staff start from facility, date, appointment, intake, and patient-flow events.
...
- A successful run produces a live minimum-necessary dashboard with queues, trends, and drill-through tasks.
...
- Late feeds display freshness warnings and never imply that missing events equal zero.
...
- **DocTypes:** `OC Front Desk Dashboard` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
