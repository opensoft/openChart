# eCQM Pre-close Projections — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Projects electronic quality-measure performance before period close using versioned logic and unresolved-data scenarios.
Topics: openchart-feature-catalog, analytics, frappe, ecqm
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-055 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Scenario Bands Can Show The** — scenario bands can show the effect of resolving specific missing-data classes.

## Focus

This feature isolates one capability: projects electronic quality-measure performance before period close using versioned logic and unresolved-data scenarios. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Quality Reporting Teams start from measure version, eligible population, exclusions, evidence, open gaps, and close date.
...
- A successful run produces projected numerator, denominator, uncertainty range, and unresolved case list.
...
- Uncertified logic or incomplete feeds prevent projection from being labeled final.
...
- **DocTypes:** `OC eCQM Projection` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
