# Referral Pattern Analysis — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Shows inbound and outbound referral flows, completion, leakage, latency, and network concentration between providers.
Topics: openchart-feature-catalog, analytics, frappe, referral-analytics
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-046 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Capacity Overlays Can Reveal Where** — capacity overlays can reveal where referral delay aligns with access constraints.

## Focus

This feature isolates one capability: shows inbound and outbound referral flows, completion, leakage, latency, and network concentration between providers. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Network Managers start from referral events, provider identities, specialties, destinations, statuses, and periods.
...
- A successful run produces permissioned flow tables and networks with completion and turnaround measures.
...
- Unmatched destinations remain unknown nodes and small flows are suppressed.
...
- **DocTypes:** `OC Referral Analysis` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
