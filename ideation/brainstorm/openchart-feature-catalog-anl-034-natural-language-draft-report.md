# Natural-language Draft Report — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Translates a user's question into a permission-scoped draft report that must be inspected before execution or publication.
Topics: openchart-feature-catalog, analytics, frappe, natural-language-reporting
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-034 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Question-To-Definition Citations Can Teach Users** — question-to-definition citations can teach users the open metric semantics selected.

## Focus

This feature isolates one capability: translates a user's question into a permission-scoped draft report that must be inspected before execution or publication. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Report Authors start from a natural-language question, user scope, catalog semantics, and optional time context.
...
- A successful run produces a draft field selection, filters, metric versions, assumptions, and cost preview.
...
- Ambiguous terms trigger clarification and the system never expands permissions or auto-publishes.
...
- **DocTypes:** `OC Report Draft` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
