# SOAP Note Editor — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Provides a structured Subjective, Objective, Assessment, and Plan authoring surface with independent section state.
Topics: openchart-feature-catalog, clinical-documentation, frappe, soap-note
Repository context: openChart — Frappe v15 native EMR; catalog entry DOC-014 (Encounters And Clinical Documentation)
Captured: 2026-08-24

## Possible feats

- **Section-specific assistants** — Could suggest drafts without signing authority.

## Focus

Provides a structured Subjective, Objective, Assessment, and Plan authoring surface with independent section state. This feature isolates that capability from scheduling, billing, and other systems that may consume its governed outputs.

## Behavior

- **Actors:** Clinician may initiate the capability only for patients and encounters allowed by Frappe user permissions.
- **Inputs:** The workflow requires encounter context, four SOAP sections, linked evidence, and authorship.
...
- **Transitions:** Draft → Ready for Review → Signed; accepted content is immutable. Every transition records actor, timestamp, and rationale where applicable.
...
- **Output:** The user receives a reviewable clinical note ready for signature; accepted clinical records are never silently rewritten.

## Frappe realization

- **DocTypes:** `OC Clinical Note` is a submittable clinical DocType with naming series `OC-.#####`; key fields are encounter, note_type, subjective, objective, assessment, plan, author, template_version.
- **Workflow:** Frappe Workflow implements Draft → Ready for Review → Signed; accepted content is immutable. After submission, corrections use open_chart.api.v1 succession records; accepted rows are never edited in place.
...
- **Surfaces:** Desk editor client script, autosave draft API, and SOAP print format; client scripts handle conditional fields, stale-revision warnings, and explicit review confirmations.
