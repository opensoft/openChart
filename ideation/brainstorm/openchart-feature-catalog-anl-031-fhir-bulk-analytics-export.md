# FHIR Bulk Analytics Export — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Produces permissioned FHIR bulk data files for standards-based analytics and downstream population workflows.
Topics: openchart-feature-catalog, analytics, frappe, fhir-bulk
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-031 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Incremental Exports Can Preserve Server** — incremental exports can preserve server watermarks for repeatable downstream loads.

## Focus

This feature isolates one capability: produces permissioned FHIR bulk data files for standards-based analytics and downstream population workflows. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Interoperability Analysts start from authorized resource types, patient scope, since timestamp, and export purpose.
...
- A successful run produces asynchronous NDJSON files, manifest, checksums, expiry, and outcome errors.
...
- Revoked authority cancels unfinished jobs and inaccessible resources are omitted with counts.
...
- **DocTypes:** `OC FHIR Bulk Export` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
