# Equity Stratification Controls — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Applies consistent race, language, insurance, geography, sex, and other approved stratifiers to any compatible metric.
Topics: openchart-feature-catalog, analytics, frappe, equity-analytics
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-059 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Intersectional Analysis Can Require Heightened** — intersectional analysis can require heightened thresholds and privacy review.

## Focus

This feature isolates one capability: applies consistent race, language, insurance, geography, sex, and other approved stratifiers to any compatible metric. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Equity Analysts and Metric Authors start from a metric, approved stratifier version, categories, reference group, and suppression rules.
...
- A successful run produces stratified results with disparity measures, uncertainty, unknowns, and definition lineage.
...
- Invalid sample size, prohibited combinations, or incompatible grain block display.
...
- **DocTypes:** `OC Equity Stratification` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
