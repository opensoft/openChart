# Scheduled Report Delivery — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Delivers reproducible report snapshots by email or SFTP on a governed schedule with recipient-level controls.
Topics: openchart-feature-catalog, analytics, frappe, report-delivery
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-011 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Delivery Windows Can Avoid Sending** — delivery windows can avoid sending sensitive reports during local off-hours.

## Focus

This feature isolates one capability: delivers reproducible report snapshots by email or SFTP on a governed schedule with recipient-level controls. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Report Owners start from a certified report, parameters, cadence, timezone, channel, and recipients.
...
- A successful run produces an encrypted attachment or secure delivery reference plus per-recipient outcome.
...
- Stale permissions, delivery failures, or oversized outputs pause delivery and notify the owner.
...
- **DocTypes:** `OC Report Schedule` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
