# Query Cost Guardrails — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Estimates query cost, enforces runtime and row limits, and explains safer alternatives before resource-heavy work runs.
Topics: openchart-feature-catalog, analytics, frappe, query-guardrails
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-041 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Personal Sandboxes Can Offer Smaller** — personal sandboxes can offer smaller sampled datasets for exploratory design.

## Focus

This feature isolates one capability: estimates query cost, enforces runtime and row limits, and explains safer alternatives before resource-heavy work runs. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Report Authors and Platform Operators start from a compiled query plan, user budget, dataset statistics, and requested output.
...
- A successful run produces a cost display and allow, background, narrow, or reject decision.
...
- Timeouts cancel work cleanly and preserve partial diagnostics without partial results.
...
- **DocTypes:** `OC Query Budget` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
