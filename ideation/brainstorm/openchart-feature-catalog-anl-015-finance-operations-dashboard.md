# Finance Operations Dashboard — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Presents charges, payments, aging, denials, and payer trends from governed financial extracts without moving billing ownership into openChart.
Topics: openchart-feature-catalog, analytics, frappe, finance-dashboard
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-015 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Reconciliation Worklists Can Route Material** — reconciliation worklists can route material source variances to finance owners.

## Focus

This feature isolates one capability: presents charges, payments, aging, denials, and payer trends from governed financial extracts without moving billing ownership into openChart. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Finance Analysts start from permissioned charge, payment, claim, denial, and payer aggregates.
...
- A successful run produces a reconciled financial dashboard with source-system timestamps and variance flags.
...
- Unreconciled periods remain provisional and cannot be represented as closed books.
...
- **DocTypes:** `OC Finance Dashboard` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
