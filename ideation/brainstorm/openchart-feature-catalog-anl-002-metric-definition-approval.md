# Metric Definition Approval — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Routes proposed metric definitions through accountable review before they become available for production reporting.
Topics: openchart-feature-catalog, analytics, frappe, metric-governance
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-002 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Impact Previews Identify Dashboards And** — impact previews identify dashboards and reports affected by a proposed definition change.

## Focus

This feature isolates one capability: routes proposed metric definitions through accountable review before they become available for production reporting. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Metric Authors and Analytics Approvers start from a draft definition, validation evidence, change rationale, and reviewer assignments.
...
- A successful run produces an approved, rejected, or superseded definition with decision evidence.
...
- Missing evidence or unresolved reviewer comments keep the version in review.
...
- **DocTypes:** `OC Metric Review` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
