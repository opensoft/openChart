# ED Utilization By Attributed Panel — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Relates emergency-department use to attributed panels with rate, repeat-use, and follow-up views.
Topics: openchart-feature-catalog, analytics, frappe, ed-utilization
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-049 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Post-Ed Follow-Up Completion Can Be** — post-ED follow-up completion can be analyzed as a separate outcome.

## Focus

This feature isolates one capability: relates emergency-department use to attributed panels with rate, repeat-use, and follow-up views. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Population Health Leaders start from ED encounters or claims, attribution periods, panel membership, and follow-up events.
...
- A successful run produces risk-adjustable utilization measures and authorized high-use cohort drill-down.
...
- Out-of-network incompleteness and attribution gaps remain visible in every result.
...
- **DocTypes:** `OC ED Utilization Snapshot` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
