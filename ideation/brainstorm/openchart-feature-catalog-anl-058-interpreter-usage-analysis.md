# Interpreter Usage Analysis — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Measures interpreter demand, fulfillment, modality, language, wait, and encounter completion to support language access.
Topics: openchart-feature-catalog, analytics, frappe, interpreter-analytics
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-058 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Capacity Forecasts Can Use Reviewed** — capacity forecasts can use reviewed demand patterns without changing appointment priority.

## Focus

This feature isolates one capability: measures interpreter demand, fulfillment, modality, language, wait, and encounter completion to support language access. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Language Access Managers start from language preference, interpreter requests, assignments, fulfillment, modality, and timing.
...
- A successful run produces suppression-aware utilization, unmet demand, wait, and outcome measures.
...
- Sensitive language groups and small cells are protected from identifiable display.
...
- **DocTypes:** `OC Interpreter Snapshot` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
