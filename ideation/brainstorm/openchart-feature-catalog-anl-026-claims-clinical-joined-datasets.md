# Claims And Clinical Joined Datasets — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Creates governed analytical views joining claims and clinical events for value-based care analysis.
Topics: openchart-feature-catalog, analytics, frappe, claims-clinical
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-026 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Reusable Episode And Attribution Views** — reusable episode and attribution views can standardize contract analyses.

## Focus

This feature isolates one capability: creates governed analytical views joining claims and clinical events for value-based care analysis. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Value-based Care Analysts start from patient matching results, claims, encounters, attribution, code mappings, and periods.
...
- A successful run produces a versioned dataset with match confidence, grain, completeness, and lineage.
...
- Ambiguous identity matches are quarantined and never silently joined.
...
- **DocTypes:** `OC Analytics Dataset` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
