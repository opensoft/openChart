# De-identified Extract Generation — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Generates governed analytical extracts with explicit de-identification transforms and re-identification risk review.
Topics: openchart-feature-catalog, analytics, frappe, deidentification
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-032 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Synthetic-Data Substitution Can Preserve Schema** — synthetic-data substitution can preserve schema for low-risk development use.

## Focus

This feature isolates one capability: generates governed analytical extracts with explicit de-identification transforms and re-identification risk review. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Privacy Analysts start from an approved dataset, purpose, field transforms, date handling, and risk thresholds.
...
- A successful run produces a de-identified package with risk assessment, transform manifest, and expiry.
...
- Risk above threshold, unsafe quasi-identifiers, or missing approval prevent release.
...
- **DocTypes:** `OC Deidentified Extract` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
