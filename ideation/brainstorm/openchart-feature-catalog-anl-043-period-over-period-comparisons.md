# Period-over-period Comparisons — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Compares current, prior, year-over-year, and custom periods using compatible metric and population definitions.
Topics: openchart-feature-catalog, analytics, frappe, time-comparison
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-043 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Event Annotations Can Explain Closures,** — event annotations can explain closures, migrations, or campaigns visible in trends.

## Focus

This feature isolates one capability: compares current, prior, year-over-year, and custom periods using compatible metric and population definitions. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Managers and Analysts start from a certified metric, comparison periods, calendar rules, and dimensional scope.
...
- A successful run produces absolute and relative change with denominator, comparability, and confidence cues.
...
- Definition or cohort changes mark periods noncomparable rather than forcing a delta.
...
- **DocTypes:** `OC Metric Comparison` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
