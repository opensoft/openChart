# Metric API Access — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Exposes certified metric values and definitions through stable, scoped APIs for reusable automation.
Topics: openchart-feature-catalog, analytics, frappe, metric-api
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-029 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Change Feeds Can Notify Consumers** — change feeds can notify consumers when public metric definitions are superseded.

## Focus

This feature isolates one capability: exposes certified metric values and definitions through stable, scoped APIs for reusable automation. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Approved Applications and BI Tools start from metric identifiers, dimensions, period, scope, and API credentials.
...
- A successful run produces versioned JSON results with pagination, freshness, lineage, and suppression metadata.
...
- Invalid scopes, expensive requests, and deprecated metric versions return structured errors.
...
- **DocTypes:** `OC Analytics API Client` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
