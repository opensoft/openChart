# Charge Lag Monitoring — Brainstorm

Status: brainstorm
Kind: architecture
Summary: Measures elapsed time from clinical service to charge capture and identifies accountable lag queues.
Topics: openchart-feature-catalog, analytics, frappe, charge-lag
Repository context: openChart — Frappe v15 native EMR; catalog entry ANL-052 (Analytics Reporting Dashboards)
Captured: 2026-08-24

## Possible feats

- **Workflow Annotations Can Distinguish Expected** — workflow annotations can distinguish expected holds from preventable delay.

## Focus

This feature isolates one capability: measures elapsed time from clinical service to charge capture and identifies accountable lag queues. It keeps analytical interpretation explicit so openChart users and pluggable open-source BI tools can share semantics without a proprietary reporting lock-in.

## Behavior

- Revenue Integrity Teams start from service timestamps, documentation completion, charge events, correction events, and ownership.
...
- A successful run produces lag distributions, aging buckets, trends, and authorized encounter worklists.
...
- Missing service evidence is a data-quality exception and not treated as zero lag.
...
- **DocTypes:** `OC Charge Lag Snapshot` uses `OC-ANL-.YYYY.-.#####` naming with owner, status, definition version, JSON parameters, source watermark, permission scope, and lineage Links; child tables hold dimensions, filters, recipients, or result references as appropriate.
