# expanded-patient-intake Specification

## Purpose
The p4 expanded-patient-intake foundation: an original standalone
Frappe application (`open_chart`) that persists patient-reported
intake — identity with namespaced external identifiers, versioned
idempotent submissions, eight statement families preserving reported
detail — behind a versioned, authorized API with no clinician or
autonomous authority. Created by archiving change
add-expanded-patient-intake-foundation (accepted 2026-08-05;
acceptance record: specs/001-expanded-patient-intake/acceptance-record.md).
## Requirements
